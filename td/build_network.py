...
What it builds (SPEC §1, §3, Phase 2 細節)::
    ...
    /project1/amv
      ...
      centroid (placeholder, Phase 3 Script CHOP) ──────────────────────────────────────┘    │
                                       feature_sender (wall clock 10 Hz) ─ osc_out ─────────────┘
...
Built and rendered in TouchDesigner 2025.33230 on 2026-09-08. Historical
# VERIFY markers remain on optional/version-dependent parameters. Unit tests
validate contracts; actual GPU/audio evidence lives in the runtime report.
Missing operator classes are logged and listed by summary; a completed build
is never itself proof that all scenes work.
"""
...
IMPLEMENTED_SCENES = ("fractal_temple", "tunnel", "particle_field", "kaleido_mesh", "projectm_blend")
...
def bootstrap_code(td_dir, module):
    ...
    return (
        ...
        "import sys, os\n"
        "_TD_DIR = os.path.join(project.folder, 'td')\n"
        "if not os.path.isfile(os.path.join(_TD_DIR, 'parspec.py')): _TD_DIR = %r\n"
...
def build_audio(amv):
    ...
    set_par(audio_in, "device", "BlackHole2ch_UID")
    set_par(audio_in, "errormissing", True)        # VERIFY par name and that the
    ...
    set_par(spectrum, "frequencylog", 0.0)
    set_par(spectrum, "highfreqboost", 0.0)
    ...
    # Send at a measured wall-clock cadence, independent of CHOP sample rate.
    osc_out = create(amv, "oscoutDAT", "osc_out", 500, 200)
    set_par(osc_out, "address", OSC_OUT_HOST)
    set_par(osc_out, "port", OSC_OUT_PORT)
    set_par(osc_out, "active", True)
    sender = create(amv, "executeDAT", "feature_sender", 650, 200)
    set_text(sender, FEATURE_SEND_BODY)
    set_par(sender, "framestart", True)
...
def onFrameStart(frame):
    ...
    source = op('features')
    kick = source['kick']
    if kick is not None: _kick = max(_kick, float(kick[0]))
    now = time.monotonic()
    if now < _next: return
    _next = (int(now * 10) + 1) / 10.0
    out = op('osc_out')
    for name in ('bass','mid','high','energy','kick','centroid'):
        channel = source[name]
        if channel is not None:
            value = _kick if name == 'kick' else float(channel[0])
            out.sendOSC('/feat/' + name, [value])
    _kick = 0.0
"""
...
def build_director(amv, specs, td_dir):
    ...
    set_text(callbacks, read_text(os.path.join(td_dir, "osc_in_callbacks.py")) + (
        ...
        "_TD_DIR = _os.path.join(project.folder, 'td')\n"
        "if not _os.path.isfile(_os.path.join(_TD_DIR, 'parspec.py')): _TD_DIR = %r\n"
    ...
    set_par(par_exec, "op ops", director.path)
    set_par(par_exec, "builtin", False)        # VERIFY Parameter Execute source par
...
MIDI_DEVICE = ""  # Optional MIDI Device Mapper table path; disabled until configured.
...
def build_midi_override(amv, td_dir=None):
    ...
    # TD 2025 Device is a DAT path, not a numeric device index.
    set_par(midi_in, "device id", MIDI_DEVICE)
    set_par(midi_in, "channel channels", MIDI_IN_CHANNEL)  # VERIFY channel filter par
    set_par(midi_in, "active", False)
...
def build_scenes(amv):
    """Five native GPU scenes sharing a single editable fragment shader."""
    shader = create(amv, "textDAT", "scene_shader", -650, 1150)
    set_text(shader, read_text(os.path.join(find_td_dir(), "shaders", "visualizer.frag")))
    scenes = {}
    for index, name in enumerate(IMPLEMENTED_SCENES):
        scene = create(amv, "glslTOP", name, -350, 1300-index*150)
        set_par(scene, "pixeldat", shader.path)
        set_par(scene, "outputresolution", "custom")
        set_par(scene, "resolutionw", 1280)
        set_par(scene, "resolutionh", 720)
        vectors = (
            ("uAudio", [feat(c) for c in ('bass','mid','high','energy')]),
            ("uClock", ["absTime.seconds", lagged('camera_speed'),
                        "op('director').par.Symmetry", str(index)]),
            ("uModes", ["op('director').par.Particlemode.menuIndex",
                        "op('director').par.Palette.menuIndex", "1280.0/720.0", feat('kick')]),
        )
        for i, (uniform, values) in enumerate(vectors):
            set_par(scene, "vec%dname" % i, uniform)
            for axis, expression in zip('xyzw', values):
                set_expr(scene, "vec%dvalue%s" % (i,axis), expression)
        scenes[name] = scene
    return scenes
...
def build_projectm_input(amv):
    ...
    * ``pm_black`` — a black Constant TOP. The *default*, and the reason
    ...
    set_expr(pm_ndi, "active", "op('director').par.Projectmsource == 'ndi'")
    ...
    set_par(pm_black, "resolutionh", 720)
...
def build_post(amv, scenes, palette_switch):
    ...
    palette = create(amv, "nullTOP", "palette", 550, 1000)
    connect(scene_switch, palette, 0)      # index image
    # Palette now lives in the shader; this Null keeps the post-chain outlet stable.
    ...
    connect(palette, feedback)
    connect(feedback, fb_xform)
    set_expr(fb_xform, "sx", "1.01 + %s * 0.04" % lagged("camera_speed"))
    set_expr(fb_xform, "sy", "me.par.sx")
    set_par(fb_xform, "extend", "mirror")
    ...
    set_par(pm_fit, "resolutionh", 720)

    projectm_level = create(amv, "levelTOP", "projectm_level", 1000, 700)
    connect(pm_fit, projectm_level)
    set_expr(projectm_level, "opacity", "0 if op('director').par.Projectmsource == 'none' else " + lagged("projectm_mix"))  # VERIFY opacity par

    composite = create(amv, "compositeTOP", "composite", 1150, 1000)
    connect(projectm_level, composite, 0)
    connect(fb_mix, composite, 1)
    set_par(composite, "outputresolution", "custom")
    set_par(composite, "resolutionw", 1280)
    set_par(composite, "resolutionh", 720)
    ...
    set_par(window, "title", "Agentic Music Visualizer")
    set_par(window, "size", "custom")
    set_par(window, "borders", True)
    set_par(window, "ignoretaskbar", True)
    set_par(window, "closeescape", True)
    set_par(window, "winw", 960)
    set_par(window, "winh", 540)

    startup = create(amv, "executeDAT", "startup", 1450, 1250)
    set_text(startup, STARTUP_BODY)
    set_par(startup, "start", True)
...
def onStart():
    root = me.parent()
    director = root.op('director')
    for key in ('manual_touch', 'pending_discrete', 'script_write', 'last_heartbeat', 'heartbeat_epoch'):
        director.unstore(key)
    director.par.Record = False
    root.op('window').par.winopen.pulse()
"""
...
def build(td_dir=None, schema_path=None):
    ...
    build_post(amv, scenes, None)

    log("shader palettes: %s" % ", ".join(PALETTES))
